...
set.seed(10)
#----split data
reg<-FetchData(data,vars = c('ident','GSN','CCL3','SDS'),slot = 'counts')
table(reg$ident)
...
table(test$ident)
#-------------------------------------------------------------------------
model<-glm(ident~GSN+CCL3+SDS,data = train, family = binomial)
summary(model)
logLik(model)
...
break 
#----Figure 6c
VlnPlot(data, features = c('GSN','CCL3','SDS'),cols = c('grey','yellow'))
FindMarkers(data, ident.1 = 'ESCC', ident.2 = 'adjacent', features = c('GSN','CCL3','SDS'))
#-------Figure 6d 
