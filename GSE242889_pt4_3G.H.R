...
library(pROC)
library(ROCR)
setwd('/home/em_b/Desktop/scRNAseq_manuscript/Fig.3/GSE242889_RAW/4NT_P29')
...
setwd('/home/em_b/Desktop/scRNAseq_manuscript/Fig.3/GSE242889_RAW/4T_C29')
features_path <- 'genes.tsv'
barcodes_path <- 'barcodes.tsv'
matrix_path <- 'matrix.mtx'
matrix <- ReadMtx(mtx= matrix_path, features = features_path, cells= barcodes_path)
...
data<-merge(x,y=c(y),project='patient_4')
...
plot.roc(actuals, predicted,
         percent = TRUE,
         main = 'HCC patient 4 Area Under the ROC curve',
         add =  FALSE,
         asp = NA,
         print.auc = TRUE,
         print.auc.col='red',
