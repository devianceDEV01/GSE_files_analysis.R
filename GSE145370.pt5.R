...
set.seed(10)
#----split data
reg<-FetchData(data,vars = c('ident','APOC1','SPP1','C1QB'),slot = 'counts')
table(reg$ident)
...
table(test$ident)
#-------------------------------------------------------------------------
model<-glm(ident~APOC1+SPP1+C1QB,data = train, family = binomial)
summary(model)
logLik(model)
...
break 
#----Figure 6i
VlnPlot(data, features = c('APOC1','SPP1','C1QB'),cols = c('grey','yellow'))
FindMarkers(data, ident.1 = 'ESCC', ident.2 = 'adjacent', features = c('APOC1','SPP1','C1QB'))
#----Figure 6j
