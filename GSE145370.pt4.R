...
set.seed(10)
#----split data
reg<-FetchData(data,vars = c('ident','CXCL9','IFITM3','FN1'),slot = 'counts')
table(reg$ident)
...
table(test$ident)
#-------------------------------------------------------------------------
model<-glm(ident~CXCL9+IFITM3+FN1,data = train, family = binomial)
summary(model)
logLik(model)
...
break 
#----Figure 6g 
VlnPlot(data, features = c('CXCL9','IFITM3','FN1'),cols = c('grey','yellow'))
FindMarkers(data, ident.1 = 'ESCC', ident.2 = 'adjacent', features = c('CXCL9','IFITM3','FN1'))
#----Figure 6h
