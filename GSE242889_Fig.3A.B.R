...
names<-c(paste0("adjacent_", 1:4788))
colnames(matrix)<-names
setwd('/home/em_b/Desktop/scRNAseq_manuscript/Fig.3/GSE242889_RAW/5T_C36')
counts2 <- readMM("matrix.mtx")
genes2 <- read_tsv("genes.tsv", col_names = FALSE)
gene_ids2 <- genes2$X2
head(gene_ids2)
cell_ids2 <- read_tsv("barcodes.tsv", col_names = FALSE)$X1
head(cell_ids2)
rownames(counts2) <- gene_ids2
colnames(counts2) <- cell_ids2
matrix2<-data.frame(counts2)
names2<-c(paste0("HCC_", 1:5689))
...
condition<-c(replicate(1000,'adjacent'),replicate(1000,'HCC'))
str(condition)
dim(Matrix)
...
EnhancedVolcano(resLRT,
                ...
                x='log2FoldChange',
                y='pvalue',
                title = 'Differential gene expressions from HCC surgical resected tumors vs paired adjacent tissue',
...
pathways<-c('GO:0002262','GO:0002274','GO:0097529','GO:0061515','GO:0002573',
            'GO:0002695','GO:0032755','GO:0032731','GO:0001819') 
