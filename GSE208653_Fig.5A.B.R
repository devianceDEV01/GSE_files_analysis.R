...
counts <- readMM("GSM6360682_N_1.matrix.mtx.gz")
genes <- read_tsv("GSM6360682_N_1.features.tsv.gz", col_names = FALSE)
gene_ids <- genes$X2
head(gene_ids)
cell_ids <- read_tsv("GSM6360682_N_1.barcodes.tsv.gz", col_names = FALSE)$X1
head(cell_ids)
rownames(counts) <- gene_ids
colnames(counts) <- cell_ids
matrix<-data.frame(counts)
names<-c(paste0("hpvNorm_", 1:16218))
colnames(matrix)<-names
counts2 <- readMM("GSM6360686_SCC_4.matrix.mtx.gz")
genes2 <- read_tsv("GSM6360686_SCC_4.features.tsv.gz", col_names = FALSE)
gene_ids2 <- genes2$X2
head(gene_ids2)
cell_ids2 <- read_tsv("GSM6360686_SCC_4.barcodes.tsv.gz", col_names = FALSE)$X1
head(cell_ids2)
rownames(counts2) <- gene_ids2
colnames(counts2) <- cell_ids2
matrix2<-data.frame(counts2)
names2<-c(paste0("hpvCC_", 1:8979))
...
condition<-c(replicate(1000,'hpvNorm'),replicate(1000,'hpvCC'))
str(condition)
dim(Matrix)
...
EnhancedVolcano(resLRT,
                ...
                x='log2FoldChange',
                y='pvalue',
                title = 'Differential gene expressions from hpv+ cervical cancer tumors vs hpv+ normal tissues',
...
go <- enrichGO(gene = genes$entrezgene_id,
               OrgDb = org.Hs.eg.db,
               ont = "BP")
pathways<-c('GO:0030099','GO:0002274','GO:0097529','GO:0002262','GO:0002573',
            'GO:0061515','GO:0002275','GO:0045637','GO:0002761') 
